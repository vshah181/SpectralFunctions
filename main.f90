...
implicit none
    ...
    integer (int32):: info, lwork, liwork, lrwork, kd, ldab, ldz
    integer (int32), allocatable :: tiwork(:), iwork(:)
    real (real64), allocatable :: trwork(:), rwork(:), energies(:), omegas(:), &
        energies_glob(:)
    complex (real64), allocatable :: twork(:), work(:), kham(:, :), ab(:, :),  &
    ...
    do ik=ibeg, iend
        ...
        if (.not.(bulk)) then
            if(.not.(allocated(ab))) allocate(ab(tot_bands, tot_bands))
            ab = kham
            kd = max(0, maxval(r_list(:, direction)))
            ldab = tot_bands
            ! 'z' is just kham
            ldz = tot_bands
            ...
            if(.not.(allocated(work) .and. allocated(rwork)                    &
                ...
                call zheevd('V', 'L', tot_bands, kham, tot_bands,              &
                    energies(ikeneb:ikenee), twork, lwork, trwork, lrwork,     &
                    tiwork, liwork, info) 
                lwork=int(twork(1))
                lrwork=int(trwork(1))
                liwork=tiwork(1)
